...
class ODataRequest implements IODataRequest
{
	 ...
	 *
	 */
	public function execute(?Closure $alterResult = null)
	{
		if (empty($this->requestUrl)) {
			throw new ODataException(Constants::REQUEST_URL_MISSING);
		}
		...
		if ($returnType) {
			$returnObj = $response->getResponseAsObject($returnType, $alterResult);
		}
		return $returnObj;
	}
	 ...
	 *         of class $returnType
	 */
	public function executeAsync($client = NULL, ?Closure $alterResult = null)
	{
		if (is_null($client)) {
		...
		// On success, return the result/response
			function ($result) use($alterResult) {
				$response = new ODataResponse(
					$this,
					(string)$result->getBody(),
					$result->getStatusCode(),
